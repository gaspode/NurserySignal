...
resource "aws_vpc_endpoint" "bedrock_runtime" {
  ...
  private_dns_enabled = true
  subnet_ids          = [local.lambda_subnet_ids[0]]
  # Reuse the existing private endpoint security group. It already permits
  # Lambda-to-interface-endpoint HTTPS and avoids a new EC2 security-group IAM
  # permission for the GitHub deployment role.
  security_group_ids = [aws_security_group.secrets_endpoint.id]
  tags               = local.common_tags
}
