...
export function SignalDetail({ signalId, apiClient, onBack }) {
  ...
  return <section>
    ...
    <div className="detail-grid">
      ...
      <DetailPanel title="Evidence & provenance"><Field label="First seen" value={formatDate(signal.created_at, true)} /><Field label="Latest update" value={formatDate(enrichment?.updated_at, true)} /><Field label="Evidence key" value={<code>{signal.documents?.[0]?.s3_key || "—"}</code>} /><Field label="Evidence checksum" value={<code>{signal.documents?.[0]?.sha256 || "—"}</code>} /><button className="button secondary" onClick={openEvidence} disabled={!signal.documents?.length}>View preserved JSON</button><p className="muted small-text">Access uses a short-lived authenticated download URL. The evidence bucket remains private.</p></DetailPanel>
